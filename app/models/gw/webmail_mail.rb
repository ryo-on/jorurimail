...
class Gw::WebmailMail
  ...
  attr_accessor :charset, :in_to, :in_cc, :in_bcc, :in_from, :in_sender,
    :in_subject, :in_body, :in_html_body, :in_format, :in_files, :tmp_id, :request_mdn
...
#  end
  ...
  def tmp_attachments
    ...
    Gw::WebmailMailAttachment.find(:all, :conditions => {:tmp_id => tmp_id})
  end
  ...
  def valid?(mode = :send)
    ...
    #errors.add_to_base "件名が未入力です。"   if in_subject.blank?
    errors.add_to_base "件名は100文字以内で入力してください。" if in_subject.size > 100
    errors.add_to_base "宛先は150件以内で入力してください。" if @in_to_addrs.size > 150
    errors.add_to_base "Ccは150件以内で入力してください。"   if @in_cc_addrs.size > 150
    errors.add_to_base "Bccは150件以内で入力してください。"  if @in_bcc_addrs.size > 150
    
    tmp_attachments.each do |f|
      errors.add_to_base "添付ファイルが見つかりません。（#{f.name}）" unless File.exist?(f.upload_path)
    end
    
    if mode == :send
      errors.add_to_base "送信元が未入力です。" if in_from.blank?
      errors.add_to_base "宛先が未入力です。"   if in_to.blank? && in_cc.blank? && in_bcc.blank?
      if in_format == FORMAT_HTML
        body_check = !in_html_body.blank?
      else
        body_check = !in_body.blank?
      end
      errors.add_to_base "本文が未入力です。" unless body_check
    end
  ...
  def single_pagination(id, params = {})
    ...
    uids = imap.uid_sort(params[:sort], params[:conditions], "utf-8").reverse
  ...
  def self.fetch(uids, mailbox, options = {})
    ...
    ## load imap
    fields = ["UID", "FLAGS", "RFC822.SIZE", "BODY.PEEK[HEADER.FIELDS (DATE FROM TO SUBJECT CONTENT-TYPE DISPOSITION-NOTIFICATION-TO)]"]
    msgs   = imap.uid_fetch(uids, fields)
    msgs.each do |msg|
      item = self.new
      item.parse(msg.attr["BODY[HEADER.FIELDS (DATE FROM TO SUBJECT CONTENT-TYPE DISPOSITION-NOTIFICATION-TO)]"])
...
protected
  ...
  def make_html_part(body, charset = 'utf-8')
    part = Mail::Part.new
    part.content_type %Q(text/html; charset="#{charset}")
    part.body modify_html_body(body)
    part
  end
